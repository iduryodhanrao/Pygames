...
class Optionscreen(Screen):
    def __init__(self, **kwargs):
        ...
        NumButton = Button(font_size=20, text="Number Puzzle", on_press=self.changer);
        bx.add_widget(NumButton)
        FamilyButton = Button(font_size=20, text="Family Puzzle",on_press=self.changer);
        bx.add_widget(FamilyButton)
        CartoonButton = Button(font_size=20, text='Cartoon Puzzle',on_press=self.changer);
        bx.add_widget(CartoonButton)
        AnimalButton = Button(font_size=20, text="Animal Puzzle",on_press=self.changer);
        bx.add_widget(AnimalButton)
        PaintButton = Button(font_size=20, text="Paint", on_press=self.changer);
        bx.add_widget(PaintButton)
        ImageShuffleButton = Button(font_size=20, text="Image shuffle", on_press=self.changer);
        bx.add_widget(ImageShuffleButton)
        CloseButton = Button(font_size=20, text="Close", on_press=self.changer);
...
class RootWidget(Widget):
    ...
    myImageList = ['ZERO.png', 'ONE.png', 'TWO.png', 'THREE.png', 'FOUR.png', 'FIVE.png', 'SIX.png',
                            'SEVEN.png', 'EIGHT.png', 'NINE.png', 'AARIT.png', 'AISHINI.png', 'AISHINI2.png', 'AISHINI3.png',
                   'AMMA.png', 'ATTA.png','KAVITA.png', 'KAVITA2.png', 'TATA.png', 'YUDI.png', 'ANNA.png', 'DONALD.png',
                   'ELSA.png', 'GOOFY.png', 'JASMINE.png', 'MERMAID.png','MICKEY.png', 'MINI.png', 'MOANA.png', 'RUPUNJAL.png',
                   'WONDERW.png', "aligator.png", "bear.png", "dog.png", "fox.png", "hippo.png", "kangaroo.png",
                            "lion.png","monkey.png", "squirrel.png", "tiger.png", "zebra.png" ]
    cardPositioned = 0
    ...
    def __init__(self, **kwargs):
        ...
        cell_width = int(Window.width/3)
        cell_height = cell_width

        for i in range(3):
            cw = ImageWidget(imgicon="images/" + self.imgList[i*2+1], setDrag=False, imgalpha=.3, pos=(i * cell_width, int(Window.height/2)),
                             size=(cell_width, cell_height))
            cw.addImage()
            self.add_widget(cw)
            cw = ImageWidget(imgicon="images/" + self.imgList[i*2], setDrag=False, imgalpha=.3,
                             pos=(i * cell_width, int(Window.height/2) - cell_height),
                             size=(cell_width, cell_height))
            cw.addImage()
            self.add_widget(cw)
        for i in range(3):
            #upper half of window
            cw = ImageWidget(imgicon="images/" + self.imgList[i*2], setDrag = True,
                             pos=(random.randint(0, Window.width - cell_width),
                                  random.randint(int(Window.height/2) + cell_height, Window.height - cell_height)),
                             size=(cell_width,cell_height))
            cw.addImage()
            self.add_widget(cw)
            #lower half of window
            cw = ImageWidget(imgicon="images/" + self.imgList[i*2+1], setDrag=True,
                             pos=(random.randint(0, Window.width - cell_width),
                                  random.randint(0, Window.height/2 - 2*cell_height)),
                             size=(cell_width,cell_height))
            cw.addImage()
            self.add_widget(cw)
    ...
    def update(self, dt):
        ...
        if self.cardPositioned >= 6:
            content = Button(text='Close')
            popup = Popup(title='You Won!!', content=content,
                          size_hint=(None,None), size=(Window.width/2,Window.height/4), auto_dismiss = False )
            content.bind(on_press=self.closeRW)
            ...
            if child.setDrag:
                for ch in self.children:
                    if child != ch and child.collide_widget(ch) and child.imgicon == ch.imgicon:
                        if (child.pos[0] >= ch.pos[0] -50 and (child.pos[0] <= ch.pos[0] + ch.width + 50)) \
                                and (child.pos[1] >= ch.pos[1] -50 and (child.pos[1] <= ch.pos[1] + ch.height + 50)):
                            child.pos = ch.pos
                            child.setDrag = False
...
class MainScreen(Screen):
    def on_pre_enter(self, *args):
        ...
        self.NumberImageList = ['ZERO.png', 'ONE.png', 'TWO.png', 'THREE.png', 'FOUR.png', 'FIVE.png', 'SIX.png',
                                'SEVEN.png', 'EIGHT.png', 'NINE.png']
        self.FamilyImageList = ['AARIT.png', 'AISHINI.png', 'AISHINI2.png', 'AISHINI3.png', 'AMMA.png', 'ATTA.png',
                                'KAVITA.png', 'KAVITA2.png', 'TATA.png', 'YUDI.png']
        self.CartoonImageList = ['ANNA.png', 'DONALD.png', 'ELSA.png', 'GOOFY.png', 'JASMINE.png', 'MERMAID.png',
                                 'MICKEY.png', 'MINI.png', 'MOANA.png', 'RUPUNJAL.png', 'WONDERW.png']
        self.AnimalImageList = ["aligator.png", "bear.png", "dog.png", "fox.png", "hippo.png", "kangaroo.png",
                                "lion.png", "monkey.png", "squirrel.png", "tiger.png", "zebra.png"]
    ...
    def __init__(self, **kwargs):
        ...
        # global gameList
        self.prevID = 10
        self.uList = ["images/blank.png", "images/blank.png", "images/blank.png", "images/blank.png", "images/blank.png", "images/blank.png" , "images/blank.png", "images/blank.png"]
        ...
        for row in range(8):
            self.button.append(
                Button(id=str(row), background_normal="images/blank.png", font_size='30', disabled=False, on_press=self.on_button_press))
            main_layout.add_widget(self.button[row])
        self.add_widget(main_layout)

    def on_button_press(self, instance):
        currID = int(instance.id) # button id pressed - 0,1,2..5
        #print(instance.background_normal)
        if instance.background_normal == "images/blank.png":
            ...
            if self.prevID != 10:  # not a first time press
                if self.button[self.prevID].background_normal == instance.background_normal:  # prev and current pressed buttons have same values
                    ...
                    if self.uList.count(self.button[self.prevID].background_normal) < 2:
                        self.button[self.prevID].background_normal = "images/blank.png"
                        self.uList[self.prevID] = "images/blank.png"
            self.prevID = currID
        else:
            if self.uList.count(instance.background_normal) < 2:
                instance.background_normal = "images/blank.png"
        if self.uList.count("images/blank.png") == 0:
            b = FloatLayout()
            l=Label(text="You Won!!", color=(0,0,1,1), font_size=100, pos_hint={'x': 0.3, 'y': .6}, size_hint=(.5, .4))
            with l.canvas:
                Color(0, 1, 0, .25)
                Rectangle(pos=self.pos,size=self.size)
            b.add_widget(l)
            b.add_widget(Button(text="Play Again?", font_size=30, pos_hint={'x': 0.2, 'y': .4}, size_hint=(.4, .2),
                                on_press=self.switchscreen))
            b.add_widget(Button(text="Close", font_size=30, pos_hint={'x': 0.3, 'y': .2}, size_hint=(.4, .2),
                                on_press=self.switchscreen))
            self.add_widget(b)
